...
###### Answer: No, because dom_cnt just considers the dominant species by code per plot
...
richness <- tree_cnt %>% 
  group_by(Plot) %>% 
  summarise(richness = n_distinct(Chojnacky_Code))
#----------------
...
dom_cnt %>% filter(Plot == "D5") %>% select(Chojnacky_Code, n)
...
#----------------
dom_cnt <- dom_cnt %>% 
  mutate(rel_abd = round((n / Ttl_Trees) * 100, 1))
#----------------

# Question 7: How many trees are there at plot A5?

dom_cnt %>% filter(Plot == "A5") %>% select(Ttl_Trees)
# Question 8: How many different tree species are there at plot D1?
dom_cnt %>% filter(Plot == "D1") %>% select(richness)
